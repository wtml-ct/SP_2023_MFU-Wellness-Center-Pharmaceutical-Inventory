<template>
        ...
        <v-row>
          <v-col>
            <v-container>
              ...
              <p>เงื่อนไขของยามีทั้งหมด&nbsp;{{ this.total }}&nbsp;รายการ</p>
              <!-- </div> -->
            </v-container>
          </v-col>
          <v-spacer></v-spacer>
  ...
  export default {
    layout: "superuser",
    middleware: ["auth"],
    //change every "units" to "conditions" 
    data: () => ({
      dialog: false,
      total: 0,
      search: "",
    ...
    methods: {
      async initialize() {
        this.$axios.get(URL + "/api/conditions/findAll").then((res) => {
    ...
    async deleteItemConfirm() {
      ...
      await this.$axios.put(URL + "/api/conditions/delete", this.editedItem);
