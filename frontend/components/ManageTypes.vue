      ...
      <v-row>
        <v-col>
          <v-container>
            ...
            <p>ประเภทของยามีทั้งหมด&nbsp;{{ this.total }}&nbsp;รายการ</p>
            <!-- </div> -->
          </v-container>
        </v-col>
        <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    async initialize() {
      this.$axios.get(URL + "/api/types/findAll").then((res) => {
        this.types = res.data;
  ...
  async deleteItemConfirm() {
    ...
    await this.$axios.put(URL + "/api/types/delete", this.editedItem);
