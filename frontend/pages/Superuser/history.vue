...
<div>
    ...
    <v-row class="mt-5">
        ...
        <v-col cols="3">
            ...
            <v-autocomplete v-model="wh" :items="whdata" return-object item-text="warehouse_name" item-value="warehouse_id" label="คลังสินค้า" rounded required outlined dense @change="onStock()"></v-autocomplete>
        </v-col>
        <v-spacer></v-spacer>
    </v-row>
    <div v-if="wh != 'ทั้งหมด'">
        <v-row class="mt-3">
            ...
            <v-col cols="3">
                ...
                <v-autocomplete v-model="item" :items="itemhave" return-object item-text="item_id" item-value="item_id" label="สินค้า" rounded required outlined dense @change="onItem()"></v-autocomplete>
            ...
            <v-col cols="3">
                ...
                <v-autocomplete v-model="item" :items="itemhave" return-object item-text="item_name" item-value="item_id" label="สินค้า" rounded required outlined dense @change="onItem()"></v-autocomplete>
            </v-col>
            <v-spacer></v-spacer>
        </v-row>
    </div>
    <div v-else>
        <v-row class="mt-3">
            <v-spacer></v-spacer>
            <h3 style="color: red">กรุณาเลือกสินค้า</h3>
            <v-spacer></v-spacer>
        </v-row>
    </div>
    <!-- Table -->
    <div>
        ...
        <v-data-table :headers="headers" :search="search" :items="product" :items-per-page="20" class="elevation-1">
            <!-- index  -->
            ...
            <template slot="item.sc_balance" slot-scope="props">
                <v-btn rounded small :color="getScBalanceColor(props.item)" class="text--center" :style="{ width: '50px' }">
                    {{ formatScBalance(props.item) }}
                </v-btn>
            </template>

            <template slot="item.status_id" slot-scope="props">
                <v-btn width="100px" small rounded :class="getStatusClass(props.item)" style="color: white" @click="toPage(props.item)">
    ...
    methods: {
        ...
        async initialize() {
            ...
            this.$axios.get("/api/warehouses/findAll").then((res) => {
                ...
                this.whdata = ["ทั้งหมด", ...filteredWarehouses];

                console.log(filteredWarehouses);
                this.wh = this.whdata[0];
            });
        ...
        onStock() {
            if (this.wh != "ทั้งหมด") {
                ...
                this.$axios
                    .get(`/api/stockcards/findHistotybywh/${this.wh.warehouse_id}`)
                    .then((res) => {
                        this.product = res.data;
        ...
        onItem() {
            ...
            this.$axios
                .get(
                    `/api/stockcards/findHistotybywhanditemid/${this.wh.warehouse_id}/${this.item.item_id}`
                )
                .then((res) => {
                    this.product = res.data;
                    console.log(this.product);
