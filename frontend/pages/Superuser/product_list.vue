...
<div>
    ...
    <v-tabs fixed-tabs show-arrows>
        ...
        <v-tab-item>
            <div class="right">
                <v-row class="mt-10">
                    ...
                    <v-spacer></v-spacer>
                    <v-col>
                        <v-text-field rounded v-model="search1" label="ค้นหา" append-icon="mdi-magnify" outlined dense>
                        </v-text-field>
                    </v-col>
                </v-row>
            </div>
        ...
        <v-tab-item>
            ...
            <v-data-table :headers="headers" :search="search2" :items="product2" :items-per-page="10" class="elevation-1">
                <!-- index  -->
                <template slot="item.count" slot-scope="props">
                    {{ props.index + 1 }}
                </template>
    ...
    <v-dialog persistent v-model="dialog" max-width="800px">
        ...
        <v-card max-height="700px" class="pa-5">
            <v-form @submit.prevent="save()">
                ...
                <v-row no-gutters>
                    ...
                    <v-col cols="4">
                        <v-autocomplete :items="typee" item-text="type_name" item-value="type_id" label="หมวดหมู่" class="field" v-model="editedItem.type_id" @change="onType()" dense outlined required></v-autocomplete>
                    </v-col>
                    ...
                    <v-col cols="4">
                        <v-autocomplete :items="unitt" label="หน่วย" class="field" item-text="unit_name" item-value="unit_id" v-model="editedItem.unit_id" outlined dense required></v-autocomplete>
                    </v-col>
                    ...
                    <v-col cols="4">
                        <v-text-field class="field" label="รหัสสินค้า" v-model="editedItem.item_id" outlined dense readonly></v-text-field>
                    </v-col>
                    ...
                    <v-col cols="4">
                        <v-text-field class="field" v-model="editedItem.item_min" label="Minimun" outlined required dense type="number" min="0" :rules="[numberOnly()]"></v-text-field>
                    </v-col>
                    ...
                    <v-col cols="4">
                        <v-text-field append class="field" label="ชื่อสินค้า" dense v-model="editedItem.item_name" outlined required></v-text-field>
                    </v-col>
                    ...
                    <v-col cols="4">
                        <v-text-field class="field" label="Maximum" v-model="editedItem.item_max" outlined required dense type="number" min="0" :rules="[numberOnly()]"></v-text-field>
                    </v-col>
                    <v-col cols="2">
                        <v-subheader style="color: black">เงื่อนไข</v-subheader>
                    </v-col>
                    <v-col cols="4">
                        <v-autocomplete :items="conn" item-text="condition_name" item-value="condition_id" label="เงื่อนไข" class="field" v-model="editedItem.condition_id" outlined required dense></v-autocomplete>
                    </v-col>
                </v-row>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        <v-card class="pa-5">
            <v-form @submit.prevent="deleteItemConfirm()">
                ...
                <v-card-actions class="mt-2">
                    <v-spacer></v-spacer>
                    <v-btn color="primary" rounded text style="width: 100px" @click="dialogDelete = false">ยกเลิก</v-btn>
